...
export async function generateMetadata(): Promise<Metadata> {
  try {
    const p = await getPublishedProgramBySlug('cna');
    return {
      title: `${p.title} | Indiana CNA Certification | Elevate for Humanity`,
    ...
    return {
      title: 'Certified Nursing Assistant (CNA) | Indiana CNA Certification | Elevate for Humanity',
      description: '6-week CNA certification program in Indianapolis. 150+ hours of classroom and clinical training. WIOA funding available.',
      alternates: { canonical: '/programs/cna' },
    };
  }
}
...
export default async function CNAPage() {
  let program;
  try {
    program = await getPublishedProgramBySlug('cna');
  } catch {
    notFound();
  }
    ...
    <main className="min-h-screen bg-white text-slate-900">
      ...
      <section className="border-t border-slate-200 bg-slate-50 py-12">
        <div className="mx-auto max-w-3xl px-6 text-center">
          ...
          <p className="mt-3 text-slate-600">WIOA funding available for eligible Indiana residents. Classes start monthly.</p>
