    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              {NAV.map((n) => (
                <Link key={n.label} href={n.href} className="px-3 py-2 text-sm font-medium text-slate-900 hover:text-brand-red-600 hover:bg-white rounded-lg transition-colors">{n.label}</Link>
              ))}
            ...
            <button onClick={() => setMobileOpen(!mobileOpen)} className="lg:hidden p-2 rounded-lg text-slate-700 hover:bg-white" aria-label="Menu">
              {mobileOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
            </button>
          </div>
          {mobileOpen && (
            <div className="lg:hidden border-t py-3 space-y-1">
              {NAV.map((n) => <Link key={n.label} href={n.href} onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 text-sm font-medium text-slate-900 hover:bg-white rounded-lg">{n.label}</Link>)}
      ...
      <section className="py-16 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-700 max-w-2xl mx-auto">Industry-recognized certifications and hands-on training. Many programs are fully funded through WIOA and state workforce grants.</p>
          </div>
      ...
      <section className="py-16 px-6">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-slate-700 max-w-2xl mx-auto">Training across five states. Select a location to see programs, schedules, and enrollment near you.</p>
          </div>
            ...
            {LOCATIONS.map((loc) => (
              <div key={loc.state} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow border border-gray-100">
                ...
                <div className="p-4">
                  ...
                  <p className="text-slate-700 text-sm mb-3">{loc.desc}</p>
                  <div className="flex flex-wrap gap-1.5 mb-4">
      ...
      <section className="py-16 px-6">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-slate-700 mb-8 max-w-2xl mx-auto">Many programs are available at no cost through WIOA, state workforce grants, DOL Registered Apprenticeships, and other funding. Self-pay options also available.</p>
