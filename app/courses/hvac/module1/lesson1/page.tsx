  ...
  const handleSubmitQuiz = async () => {
    ...
    if (quizPassed) {
      ...
      try {
        ...
        // Completion tracking is best-effort
      }
    }
  }
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 pt-10">
        ...
        <p className="text-sm text-slate-500 mt-2">
          Watch the full lesson video. The instructor covers every component, the inspection
          walkthrough, and reviews the quiz questions.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10 space-y-16">
        {/* ══════════════════════════════════════════════
            ...
            ══════════════════════════════════════════════ */}
        <section>
          <SectionHeader number={1} title="What Is an HVAC System?" />
        ...
        {/* ══════════════════════════════════════════════
            ...
            ══════════════════════════════════════════════ */}
        <section>
          <SectionHeader number={2} title="Condenser Unit Components" />
          <p className="text-slate-600 mb-6">
            A residential condenser unit contains six major components. Each one is explained below
            with its function and how technicians inspect it.
          </p>
          ...
          <div className="space-y-4">
            {[
              {
                name: 'Compressor',
                desc: 'The compressor is the heart of the refrigeration cycle. It receives low-pressure, low-temperature refrigerant gas from the indoor evaporator coil and compresses it into high-pressure, high-temperature gas. This pressurized gas then flows through the condenser coil where it releases heat to the outdoor air.',
                inspect:
              ...
              {
                name: 'Condenser Coil',
                desc: 'A network of copper tubing with aluminum fins that wraps around the inside of the condenser cabinet. Hot refrigerant gas from the compressor flows through the coil. As the fan pulls outdoor air across the fins, the refrigerant releases heat and changes from gas to liquid.',
                inspect:
              ...
              {
                name: 'Fan Motor',
                desc: 'Drives the fan blade that pulls outdoor air across the condenser coil. Without airflow, the coil cannot reject heat and the system will overheat. Mounted at the top of the unit behind a protective grille.',
                inspect:
              ...
              {
                name: 'Capacitor',
                desc: 'Stores and releases electrical energy to start and run the compressor and fan motors. A dual-run capacitor serves both motors. Capacitors are the single most common failure point in residential HVAC.',
                inspect:
              ...
              {
                name: 'Contactor',
                desc: 'An electrically controlled switch. When the thermostat calls for cooling, it sends 24V to the contactor coil, which pulls the contacts closed and connects 240V line voltage to the compressor and fan motor.',
                inspect:
              ...
              {
                name: 'Service Valves',
                desc: 'Access ports on the refrigerant lines where they exit the condenser. The larger line (suction) carries low-pressure gas. The smaller line (liquid) carries high-pressure liquid. Technicians connect gauge manifolds here to measure pressures and charge refrigerant.',
                inspect:
              ...
              <div key={comp.name} className="rounded-xl border bg-white shadow-sm overflow-hidden">
                <div className="px-5 py-4">
                  <h3 className="text-lg font-bold text-slate-900">{comp.name}</h3>
                  <p className="text-sm text-slate-700 mt-2 leading-relaxed">{comp.desc}</p>
        ...
        {/* ══════════════════════════════════════════════
            SECTION 3 — 3D Equipment Lab
            ══════════════════════════════════════════════ */}
        <section>
          <SectionHeader number={3} title="Interactive 3D Equipment Lab" />
          <p className="text-slate-600 mb-4">
            Click directly on any part of the condenser unit to identify it. The selected component
            will highlight and show its description and common problems. Drag to rotate, scroll to
        ...
        {/* ══════════════════════════════════════════════
            SECTION 4 — Technician Inspection Walkthrough
            ══════════════════════════════════════════════ */}
        <section>
          <SectionHeader number={4} title="Technician Inspection Walkthrough" />
          <p className="text-slate-600 mb-6">
            When a technician arrives at a service call for a condenser unit that is not cooling,
            they follow a systematic inspection process.
          </p>
        ...
        {/* ══════════════════════════════════════════════
            ...
            ══════════════════════════════════════════════ */}
        <section>
          <SectionHeader number={5} title="Knowledge Check" />
          <p className="text-slate-600 mb-6">
          ...
          {quizSubmitted && (
            <div
              className={`mt-6 rounded-xl border-2 p-5 text-center ${
                quizPassed ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'
              }`}
            >
              <div className="text-3xl mb-2">{quizPassed ? '\u2713' : '\u2717'}</div>
              <div
                className={`font-bold text-lg ${
                  quizPassed ? 'text-green-800' : 'text-red-800'
                }`}
              >
                {quizPassed ? 'Quiz Passed' : 'Quiz Not Passed'}
              </div>
              <p
                className={`text-sm mt-1 ${quizPassed ? 'text-green-700' : 'text-red-700'}`}
              >
                You scored {quizScore} out of {quiz.length}.
                {quizPassed
                  ? ''
                  : ' You need at least 4 correct. Review the material and try again.'}
              </p>
              {!quizPassed && (
                <button
                  onClick={() => {
          ...
          <section>
            <div className="rounded-2xl border-2 border-green-400 bg-green-50 p-8 text-center">
              ...
              <p className="text-green-700 mt-2 max-w-lg mx-auto">
                You watched the lesson video, identified all components on the 3D condenser model,
                completed the technician inspection walkthrough, and passed the knowledge check.
              </p>
