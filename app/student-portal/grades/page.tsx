    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        ...
        <div className="mt-4 mb-8">
          ...
          <p className="text-gray-600 mt-1">Your academic performance across all programs</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl border p-5 text-center">
            ...
            <p className="text-xs text-gray-500 mt-0.5">Courses Completed</p>
          </div>
          <div className="bg-white rounded-xl border p-5 text-center">
          ...
          <div className="bg-white rounded-xl border p-5 text-center">
            ...
            <p className="text-2xl font-bold text-purple-600">{passedCheckpoints.length}<span className="text-sm text-gray-400">/{scores.length}</span></p>
            <p className="text-xs text-gray-500 mt-0.5">Checkpoints Passed</p>
          </div>
          <div className="bg-white rounded-xl border p-5 text-center">
        ...
        {!hasAnyData && (
          <div className="text-center py-16 border rounded-xl">
            ...
            <p className="text-gray-500 text-sm mb-4">Complete lessons and quizzes to see your progress here.</p>
          ...
          <section className="rounded-xl border p-6 mb-6">
            ...
            <div className="space-y-3">
                ...
                return (
                  <div key={e.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="text-right flex-shrink-0">
                      ...
                      {e.completed_at && <p className="text-xs text-gray-400 mt-1">{formatDate(e.completed_at)}</p>}
                    </div>
                  </div>
                );
              })}
              {progEnrollments.map((e: any) => (
                <div key={e.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  <p className="font-medium text-gray-900">{e.program?.title ?? 'Program'}</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Lesson</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Course</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Score</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Result</th>
                    <th className="py-2.5 font-medium text-gray-500">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {scores.map((s: any) => (
                    <tr key={s.id} className="border-b last:border-0">
                      ...
                      <td className="py-3 pr-4 text-gray-500">{s.course?.title ?? '--'}</td>
                      <td className="py-3 pr-4">
                        <span className={`font-semibold ${s.score >= 90 ? 'text-brand-green-600' : s.score >= 70 ? 'text-brand-blue-600' : 'text-red-600'}`}>
                          {s.score}%
                        </span>
                        <span className="text-gray-400 text-xs ml-1">(pass: {s.passing_score}%)</span>
                      </td>
                      <td className="py-3 pr-4">
                        {s.passed
                      ...
                      <td className="py-3 text-gray-400">{formatDate(s.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section className="rounded-xl border p-6 mb-6">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Assignment</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Course</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Score</th>
                    <th className="py-2.5 pr-4 font-medium text-gray-500">Grade</th>
                    <th className="py-2.5 font-medium text-gray-500">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {grades.map((g: any) => (
                    <tr key={g.id} className="border-b last:border-0">
                      <td className="py-3 pr-4 font-medium text-gray-900">{g.assignments?.title ?? 'Assignment'}</td>
                      <td className="py-3 pr-4 text-gray-500">{g.courses?.name ?? g.courses?.code ?? '--'}</td>
                      <td className="py-3 pr-4">
                        {g.score !== null && g.max_score
                          ? <span>{g.score}/{g.max_score} <span className="text-gray-400">({Math.round((g.score / g.max_score) * 100)}%)</span></span>
                      ...
                      <td className="py-3 text-gray-400">{g.graded_at ? formatDate(g.graded_at) : '--'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {progress.length > 0 && (
          <section className="rounded-xl border p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              ...
              <span className="ml-auto text-sm font-normal text-gray-400">{progress.length} total</span>
            </h2>
              ...
              {progress.map((p: any) => (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700">{p.lesson_id}</span>
                  </div>
