    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Instructional Authority Hierarchy</h2>
          ...
          <div className="mt-10 bg-amber-50 border border-amber-200 rounded-lg p-6">
            ...
            <dl className="space-y-3 text-sm">
              <div>
                ...
                <dd className="text-slate-900 ml-4">A state-approved or licensed instructional authority whose credentials match the occupation they teach. Credential Partners are the primary RTI providers and may issue industry certifications.</dd>
              </div>
              <div>
                <dt className="font-semibold text-slate-900">Program Holder</dt>
                <dd className="text-slate-900 ml-4">An authorized RTI coordinator who manages curriculum delivery under the instructional supervision of a Credential Partner. Program Holders are not independent instructors — they coordinate scheduling, module delivery, and progress documentation.</dd>
              </div>
              <div>
                <dt className="font-semibold text-slate-900">Subject Matter Expert (SME)</dt>
                <dd className="text-slate-900 ml-4">An industry professional with occupation-specific credentials who provides supplemental instruction, mentoring, and lab support. SMEs are never the sole RTI authority for any program.</dd>
              </div>
              <div>
                <dt className="font-semibold text-slate-900">Employer Partner</dt>
                <dd className="text-slate-900 ml-4">An approved employer providing structured OJT under a documented work process schedule with designated supervisors and periodic evaluations.</dd>
              </div>
            </dl>
          </div>
        </div>
                  ...
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-slate-700">{tier.role}</span>
                      </div>
                      <h3 className="text-lg font-bold text-slate-900">{tier.label}</h3>
                    </div>
                  </div>
                  <p className="text-slate-900 text-sm mb-4">{tier.description}</p>
                  <div className="mb-3">
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-white">
                  ...
                  <th className="text-center p-3 font-semibold text-slate-900 border-b">Total Hours</th>
                  <th className="text-center p-3 font-semibold text-slate-900 border-b">RTI Hours</th>
                  <th className="text-center p-3 font-semibold text-slate-900 border-b">OJT Hours</th>
              ...
              <tbody>
                {RTI_PROGRAMS.map((p, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-white/50'}>
                    ...
                    <td className="p-3 text-center text-slate-900 border-b">{p.totalHours.toLocaleString()}</td>
                    <td className="p-3 text-center text-brand-blue-700 font-medium border-b">{p.rtiHours}</td>
                    <td className="p-3 text-center text-slate-900 border-b">{p.ojtHours.toLocaleString()}</td>
                    <td className="p-3 text-slate-900 border-b">{p.method}</td>
                    <td className="p-3 text-slate-900 border-b">{p.credential}</td>
                    <td className="p-3 text-slate-900 border-b text-xs">{p.credentialIssuer}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-12">
        <div className="max-w-5xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Authorized RTI Provider Registry</h2>
          <p className="text-slate-700 mb-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-white">
                  <th className="text-left p-3 font-semibold text-slate-900 border-b">Program</th>
                  <th className="text-left p-3 font-semibold text-slate-900 border-b">Credential Partner</th>
                  <th className="text-left p-3 font-semibold text-slate-900 border-b">Partner Type</th>
                  <th className="text-left p-3 font-semibold text-slate-900 border-b">Program Holder</th>
                  <th className="text-center p-3 font-semibold text-slate-900 border-b">RTI Hours</th>
                  <th className="text-center p-3 font-semibold text-slate-900 border-b">MOU</th>
                  <th className="text-left p-3 font-semibold text-slate-900 border-b">Credential Issued</th>
                </tr>
              </thead>
              <tbody>
                {RTI_REGISTRY.map((r, i) => (
                  <tr key={i} className={i % 2 === 0 ? 'bg-white/50' : 'bg-white'}>
                    ...
                    <td className="p-3 text-slate-900 border-b">{r.credentialPartner}</td>
                    <td className="p-3 text-slate-900 border-b text-xs">{r.partnerType}</td>
                    <td className="p-3 text-slate-900 border-b">{r.programHolder}</td>
                    ...
                    <td className="p-3 text-slate-900 border-b text-xs">{r.credentialIssued}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Competency Tracking & Assessment</h2>
          <p className="text-slate-700 mb-8">
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Credential Issuance (Dual-Issuer Model)</h2>
          <p className="text-slate-700 mb-6">
            ...
            {[
              ...
              <div key={i} className={`rounded-lg border p-5 ${item.color}`}>
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-slate-700 sm:ml-auto">Issued by: {item.issuer}</span>
                </div>
                <p className="text-slate-900 text-sm mb-2">{item.examples}</p>
                <p className="text-slate-700 text-xs italic">{item.note}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">Registration & Compliance References</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-lg p-5">
              <p className="text-xs text-slate-700 uppercase tracking-wider mb-1">Registered Apprenticeship Sponsor</p>
              <p className="font-semibold text-slate-900">RAPIDS ID: 2025-IN-132301</p>
              <p className="text-sm text-slate-700 mt-1">Sponsor of Record: 2Exclusive LLC-S (DBA Elevate for Humanity Career &amp; Technical Institute)</p>
            </div>
            <div className="bg-white rounded-lg p-5">
              <p className="text-xs text-slate-700 uppercase tracking-wider mb-1">ETPL / INTraining</p>
              <p className="font-semibold text-slate-900">Location ID: 10004621</p>
              <p className="text-sm text-slate-700 mt-1">Indiana Department of Workforce Development</p>
            </div>
          </div>
