...
async function _POST(req: NextRequest) {
  ...
  try {
    ...
    logger.info('[Acknowledge Handbook] Success', { userId: user.id, fullName, title });

    return Response.json({ success: true, message: 'Handbook acknowledgement recorded', data });
  } catch (err) {
    return safeInternalError(err as Error, 'Acknowledge handbook error');
  }
}
