    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-700 mt-2">Create and manage courses for your programs</p>
        </div>
      ...
      <div className="flex gap-1 mb-8 bg-gray-100 rounded-lg p-1 w-fit">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'ai' ? 'bg-white shadow text-slate-900' : 'text-slate-700 hover:text-slate-900'}`}
        >
          AI Course Generator
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-sm font-medium text-slate-700 mb-2">Published</h3>
          <p className="text-3xl font-bold text-brand-green-600">{publishedCount}</p>
        </div>
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-sm font-medium text-slate-700 mb-2">Drafts</h3>
      ...
      <div className="bg-white rounded-lg shadow-sm border">
        ...
        {filteredCourses.length > 0 ? (
            ...
            {filteredCourses.map((course) => (
              <div key={course.id} className="px-6 py-4 hover:bg-gray-50 flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-700 mt-1">{course.description?.substring(0, 100) || 'No description'}</p>
          ...
          <div className="px-6 py-12 text-center">
            <p className="text-slate-700 mb-4">{searchTerm ? 'No courses match your search' : 'No courses found'}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b flex items-center justify-between">
              ...
              <button onClick={() => { setShowCreateModal(false); resetForm(); }} className="text-slate-700 hover:text-slate-700">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-900 mb-1">Course Title *</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="publish_immediately" className="text-sm text-slate-900">Publish immediately</label>
              </div>
