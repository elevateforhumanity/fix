...
export default async function OperationsPage() {
  ...
  const [
    ...
    db.from('audit_logs').select('id, action, resource_type, created_at, user_id').order('created_at', { ascending: false }).limit(20),
    db.from('program_enrollments').select('id', { count: 'exact', head: true }).gte('enrolled_at', todayISO),
    db.from('applications').select('id', { count: 'exact', head: true }).eq('status', 'pending'),
    db.from('review_queue').select('id', { count: 'exact', head: true }).eq('status', 'pending'),
    db.from('profiles').select('id', { count: 'exact', head: true }),
    db.from('enrollment_jobs').select('id, status, created_at, error_message').order('created_at', { ascending: false }).limit(10),
    db.from('follow_ups').select('id, type, due_date, status, notes').order('due_date', { ascending: true }).limit(10),
  ]);
