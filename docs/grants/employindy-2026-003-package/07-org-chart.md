---
ELEVATE FOR HUMANITY
DOL Registered Apprenticeship Sponsor | ETPL Provider | WIOA/WRG/JRI Approved
WorkOne Partner | EmployIndy Partner | SAM.gov Registered (CAGE: 0Q856)
ByBlack Certified | IMM Certified | CareerSafe OSHA Provider
www.elevateforhumanity.org | elevate4humanityedu@gmail.com
Indianapolis, Indiana
---

# RFP 2026-003 — WIOA In-School Youth Service Provision
# Organizational Chart

---

## Governance

| Role | Name |
|---|---|
| Board of Directors | Governance & Fiduciary Oversight |
| Founder & Chief Executive Officer | Elizabeth Greene |

---

## Program Staff

| Name | Role | FTE | Location |
|---|---|---|---|
| Elizabeth Greene | Founder & CEO / Program Director | 0.25 | Elevate HQ |
| Ameco Martin | Career Coach | 1.0 | Warren Central High School (embedded) |
| Case Manager | To Be Hired upon award | 1.0 | Elevate HQ / Field |
| Alberta Davis | Testing Center Coordinator | As needed | Elevate Testing Center |

---

## Reporting Structure

| Level | Name / Role | Responsibility |
|---|---|---|
| 1 — Governance | Board of Directors | Fiduciary oversight, organizational policy |
| 2 — Executive | Elizabeth Greene, CEO | Program oversight, EmployIndy coordination, compliance, reporting |
| 3 — Direct Service | Ameco Martin, Career Coach | In-person coaching at Warren Central, ISS development, ICC data entry, employer coordination |
| 3 — Support | Case Manager | WIOA enrollment, follow-up, barrier coordination, supportive services |
| 4 — Testing | Alberta Davis | Credential exam administration, proctor services |

---

## School Partner Contacts

| Name | Title | Phone | Email |
|---|---|---|---|
| Masimba Taylor | Principal, Warren Central High School | 317-532-6203 | mtaylor4@warren.k12.in.us |
| Nicole Simpson | Intervention Specialist, Dean's Office | 317-532-6200 ext. 4687 | nsimpson@warren.k12.in.us |

---

## Employer Partner

| Organization | Role | DOL Provider ID |
|---|---|---|
| 2Exclusive Platinum Cleaning | OJT host, hiring, mentoring, on-site supervision | 206251 |

---

## WIOA Program Reporting Line

| Step | Party | Action |
|---|---|---|
| 1 | EmployIndy — Workforce Development Board | Contract oversight, performance monitoring |
| 2 | Elizabeth Greene — Program Director | Monthly meetings with EmployIndy POC, quarterly reports |
| 3 | Ameco Martin — Career Coach | Direct service delivery at Warren Central |
| 4 | Case Manager | ICC data entry, follow-up, supportive services |
| 5 | Participants | Warren Central High School students ages 16-19 |

---

## Certifications & Registrations

| Certification | Status |
|---|---|
| DOL Registered Apprenticeship Sponsor | Active — Provider ID 206251 |
| ETPL Provider (Indiana) | Active |
| SAM.gov Registration | Active — CAGE: 0Q856 |
| IMM Certification (Woman-Owned, Black-Led) | Active |
| ByBlack Certification | Active |
| WIOA / WRG / JRI Approved | Active |
| CareerSafe OSHA Provider | Active |
| Certiport CATC (Authorized Testing Center) | Active |
| NRF Rise Up Provider | Active |
| Milady Partner | Active |
| EmployIndy Partner | Active |
| WorkOne Partner | Active |

---

## Leadership Biography

**Elizabeth Greene — Founder & Chief Executive Officer**

U.S. Army Veteran · IRS Enrolled Agent (EA) · EFIN and PTIN Holder · EPA 608 Certified Proctor · Indiana Substitute Teacher · Licensed Barber · DOL Registered Apprenticeship Sponsor · SAM.gov Registered Federal Government Contractor (CAGE: 0Q856) · ByBlack Certified · IMM Certified · EmployIndy Partner · WorkOne Partner · ETPL Provider · WIOA/WRG/JRI Approved · Job Ready Indy Partner · HSI Affiliate · CareerSafe OSHA Provider · Milady Partner · NRF Rise Up Provider · Certiport CATC

Elizabeth founded Elevate for Humanity to eliminate barriers between Indianapolis residents and living-wage employment. She also operates SupersonicFastCash (tax preparation software) and Selfish Inc., a 501(c)(3) nonprofit (DBA: The Rise Foundation) providing VITA free tax preparation and community services.

---

*Elevate for Humanity | www.elevateforhumanity.org | Indianapolis, Indiana*
*elevate4humanityedu@gmail.com | SAM.gov CAGE: 0Q856*
