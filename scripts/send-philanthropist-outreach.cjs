#!/usr/bin/env node
/**
 * Philanthropic decision-maker outreach — new contacts only
 * (CICF and Lilly Endowment already sent — skipped here)
 * Usage: SENDGRID_API_KEY=SG.xxx node scripts/send-philanthropist-outreach.cjs
 */
...
// New contacts only — CICF (info@cicf.org) and Lilly (info@lei.org) already sent
const RECIPIENTS = [
  {
    org: 'Glick Philanthropies',
    to: 'info@glickphilanthropies.org',
    toName: 'Glick Philanthropies Team',
  },
];

// Pacers Foundation has no public email — contact form only
const MANUAL = [
  {
    org: 'Pacers Foundation / Herb Simon Family Foundation',
    note: 'No public email — contact form at pacersfoundation.org/contact | (317) 917-2500',
  },
];
...
async function main() {
  console.log(`Sending ${RECIPIENTS.length} new foundation email(s)...\n`);
  console.log('  (CICF and Lilly Endowment already sent — skipped)\n');
  const results = [];
