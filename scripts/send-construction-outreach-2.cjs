#!/usr/bin/env node
/**
 * Construction employer & apprenticeship outreach — batch 2
 * Usage: SENDGRID_API_KEY=SG.xxx node scripts/send-construction-outreach-2.cjs
 */
...
I would welcome the opportunity to schedule a brief meeting to introduce our organization, learn more about your programs, and explore how we can collaborate to support construction workforce development in Indiana.
...
const RECIPIENTS = [
  {
    org: 'Tonn and Blank Construction',
    to: 'contact@tonnandblank.com',
    toName: 'Tonn and Blank Construction Team',
  },
  {
    org: 'Dilling Group',
    to: 'recruiting@dillinggroup.com',
    toName: 'Dilling Group Recruiting Team',
    cc_extra: 'info@dillinggroup.com',
  },
  {
    org: 'NCW Construction Staffing',
    to: 'bbalog@teamncw.com',
    toName: 'NCW Construction Staffing Team',
  },
];

// Messer, IEC Indy, Bricklayers Local 4, ISBCTC — no direct email found, noted below
const NO_EMAIL = [
  { org: 'Messer Construction', note: 'No public email — use contact form at messer.com or call (317) 576-9250' },
  { org: 'IEC Indy', note: 'No public email — use contact form at iec-indy.org or call (317) 562-1102' },
  { org: 'Bricklayers & Allied Craftworkers Local 4', note: 'No public email — call (800) 322-2830' },
  { org: 'Indiana State Building & Construction Trades Council', note: 'No public email — use contact form at isbctc.org' },
];

function sendEmail(recipient) {
  const body = emailBody(recipient.org);
  const cc = [{ email: 'elevate4humanityedu@gmail.com', name: 'Elizabeth Greene' }];
  if (recipient.cc_extra) cc.push({ email: recipient.cc_extra, name: recipient.org });
...
async function main() {
  ...
  NO_EMAIL.forEach(n => console.log(`  ${n.org}: ${n.note}`));
}
