#!/usr/bin/env node
/**
 * Outpatient clinic & urgent care outreach — MA / Phlebotomy
 * Usage: SENDGRID_API_KEY=SG.xxx node scripts/send-clinic-outreach.cjs
 */
...
Our organization works with workforce partners including WorkOne centers, Job Ready Indy, and workforce initiatives aligned with U.S. Department of Labor programs.

In addition to workforce training, Elevate for Humanity operates a credential testing center and serves as an approved proctoring site for nationally recognized assessments including ACT WorkKeys and NRF RISE credentials.

As we expand our healthcare workforce programs, including Medical Assistant and Phlebotomy training pathways, we are seeking to build employer partnerships with healthcare providers, outpatient clinics, urgent care centers, and diagnostic facilities.
...
Our goal is to prepare individuals with foundational healthcare training and connect them directly with employers seeking skilled medical assistants and clinical support staff.
...
const RECIPIENTS = [
  { org: 'Franciscan Health',  to: 'community@franciscanalliance.org', toName: 'Franciscan Health Community Team' },
  { org: 'Riverview Health',   to: 'info@riverview.org',               toName: 'Riverview Health Team' },
];

const MANUAL = [
  { org: 'MedExpress Urgent Care',   note: 'Call (888) 345-4687 | medexpress.com — ask for Regional HR or Workforce Partnerships' },
  { org: 'NextCare Urgent Care',     note: 'Call (888) 381-4858 | nextcare.com — ask for Regional HR or Workforce Partnerships' },
  { org: 'DaVita',                   note: 'Call (800) 400-8331 | davita.com — ask for Regional Recruiting or Workforce Partnerships' },
  { org: 'Fresenius Medical Care',   note: 'Call (800) 881-5101 | freseniusmedicalcare.com — ask for Regional HR' },
];
...
function sendEmail(recipient) {
  ...
  const payload = JSON.stringify({
    ...
    subject: 'Healthcare Workforce Partnership Opportunity — Elevate for Humanity',
    content: [
...
async function main() {
  console.log(`Sending ${RECIPIENTS.length} clinic outreach emails...\n`);
  const results = [];
