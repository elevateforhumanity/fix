#!/usr/bin/env node
/**
 * Philanthropic foundation outreach — workforce ecosystem partners
 * Usage: SENDGRID_API_KEY=SG.xxx node scripts/send-foundation-outreach.cjs
 */
...
Our organization collaborates with workforce partners including WorkOne centers, Job Ready Indy, and initiatives aligned with U.S. Department of Labor workforce programs. In addition to training programs, we operate a credential testing center and serve as an approved proctoring site for nationally recognized assessments including ACT WorkKeys and NRF RISE credentials.

Our mission is to create direct pathways from training into sustainable employment across sectors including healthcare, skilled trades, manufacturing, and transportation.

As we continue expanding workforce pathways for Indianapolis residents, we are seeking to connect with philanthropic partners that share an interest in strengthening workforce access and economic mobility in our communities.

We would welcome the opportunity to introduce our organization and explore potential collaboration opportunities aligned with workforce development and community impact initiatives.
...
const RECIPIENTS = [
  {
    ...
    to: 'info@cicf.org',
    toName: 'Central Indiana Community Foundation Team',
  },
  {
    org: 'Lilly Endowment',
    to: 'info@lei.org',
    toName: 'Lilly Endowment Team',
  },
  {
    org: 'Nina Mason Pulliam Charitable Trust',
    to: 'info@ninapulliamtrust.org',
    toName: 'Nina Mason Pulliam Charitable Trust Team',
  },
  // Indianapolis Foundation shares CICF contact — skip duplicate
];
...
async function main() {
  console.log(`Sending ${RECIPIENTS.length} foundation outreach emails...\n`);
  const results = [];
