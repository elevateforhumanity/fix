#!/usr/bin/env node
/**
 * HVAC employer & apprenticeship partner outreach
 * Usage: SENDGRID_API_KEY=SG.xxx node scripts/send-hvac-outreach.cjs
 */
...
const RECIPIENTS = [
  { org: 'Mechanical Contractors Association of Indiana', to: 'info@mcaindiana.org',  toName: 'MCA Indiana Team' },
  { org: 'Indiana HVAC Association',                     to: 'info@inhvacr.org',      toName: 'Indiana HVAC Association Team' },
];

const MANUAL = [
  { org: 'Plumbers and Steamfitters Local 440', note: 'No public email — call (317) 872-1622 | 7920 E 30th St, Indianapolis' },
  { org: 'Williams Comfort Air',                note: 'No public email — call (317) 268-5900 | williamscomfortair.com' },
  { org: 'Service Plus Heating Cooling & Plumbing', note: 'No public email — call (317) 434-2627 | serviceplusnow.com' },
];
...
function sendEmail(recipient) {
  ...
  const payload = JSON.stringify({
    ...
    subject: 'HVAC Workforce Partnership Opportunity — Elevate for Humanity',
    content: [
...
async function main() {
  console.log(`Sending ${RECIPIENTS.length} HVAC outreach emails...\n`);
  const results = [];
