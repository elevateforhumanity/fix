...
-- program_holder_courses already has syllabus_path + syllabus_bucket from migration 3.
-- training_courses already has default_syllabus_path + default_syllabus_bucket from migration 5.
-- This migration adds the storage helper function.
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE t.typname = 'course_status' AND e.enumlabel = 'pending_review'
  ) THEN
    -- Add pending_review and rejected to existing status enums if they exist,
    -- otherwise add as CHECK constraint columns
    ALTER TABLE training_courses
...
-- Safe fallback: add columns if they don't exist regardless
ALTER TABLE training_courses
  ADD COLUMN IF NOT EXISTS review_status text NOT NULL DEFAULT 'draft';
ALTER TABLE curriculum_lessons
  ADD COLUMN IF NOT EXISTS review_status text NOT NULL DEFAULT 'draft';
...
-- Link exam_authorizations to pathway
...
-- Align PRS curriculum_lessons.module_id to correct modules(id) values
