...
export default function PricingTable() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-10 md:py-14">
      <h2 className="text-2xl font-bold text-black md:text-3xl">Pricing Models</h2>
        ...
        {tiers.map((t) => (
          ...
          >
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-black">{t.label}</h3>
              {t.badge ? (
            ...
            <p className="mt-2 text-2xl font-bold text-black">{format(currency, t.price)}</p>
            {t.perLearnerAt ? (
              <p className="mt-1 text-xs text-slate-500">
                {perLearnerLabel(currency, t.price, t.perLearnerAt)}
              </p>
            ) : null}
            <p className="mt-2 text-sm text-black">{t.summary}</p>
            <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-black">
      ...
      {apps.length > 0 && (
        <div className="mt-12">
          <h3 className="text-xl font-bold text-black">App-Level Pricing (if purchased separately)</h3>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-3 md:grid-cols-2">
            {apps.map((app) => {
              ...
              return (
                ...
                >
                  <h4 className="text-base font-semibold text-black">{app.label}</h4>
                  <p className="mt-1 text-sm text-black">{app.summary}</p>
                  ...
                  {app.highlights && app.highlights.length > 0 && (
                    <ul className="mt-2 list-disc space-y-1 pl-5 text-xs text-black">
      ...
      {addOns.length > 0 && (
        <div className="mt-12">
          <h3 className="text-xl font-bold text-black">Optional Add-Ons</h3>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            {addOns.map((a) => (
              <div key={a.key} className="rounded-2xl bg-white p-5 ring-1 ring-slate-200 shadow-sm">
                <div className="flex items-center justify-between">
                  <p className="text-base font-semibold text-black">{a.label}</p>
                  <p className="text-sm font-bold text-black">{format(currency, a.price)}</p>
                </div>
                <p className="mt-1 text-sm text-black">{a.summary}</p>
