import { Shield, Award, Building2, FileCheck, Briefcase, GraduationCap } from 'lucide-react';
    ...
    <section className="py-12 md:py-16 bg-white border-t border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 md:px-6">
        <div className="text-center mb-8">
          <p className="text-xs font-semibold text-brand-blue-600 uppercase tracking-widest mb-2">Institutional Authority</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {AUTHORITIES.map((a) => (
            <div key={a.abbr} className={`${a.bg} border ${a.border} rounded-xl p-4 text-center`}>
              <div className={`w-10 h-10 rounded-lg ${a.bg} flex items-center justify-center mx-auto mb-3`}>
  ...
  return (
    <section className="py-10 bg-gray-50 border-t border-gray-100">
      <div className="max-w-5xl mx-auto px-4">
        <p className="text-xs font-semibold text-brand-blue-600 uppercase tracking-widest mb-2 text-center">Credential Pipeline</p>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 md:gap-0">
          {steps.map((s, i) => (
            <div key={s.step} className="flex items-center gap-3 md:flex-col md:text-center md:gap-2 flex-1">
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block w-full h-0.5 bg-brand-blue-200 mt-5 mx-2" />
              )}
            </div>
          ))}
        </div>
