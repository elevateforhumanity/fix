...
export default function AvatarGuide({
  ...
  useEffect(() => {
    if (currentStep >= 0 && steps[currentStep]?.highlight) {
      ...
      if (element) {
        ...
        element.classList.add('ring-4', 'ring-orange-500', 'ring-offset-2', 'transition-all');
        
        return () => {
          element.classList.remove('ring-4', 'ring-orange-500', 'ring-offset-2', 'transition-all');
        };
      }
    }
  }, [currentStep, steps]);
  ...
  if (!isVisible) {
    return (
      <button
        ...
        className="fixed top-20 right-4 z-50 flex items-center gap-2 bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-full shadow-lg transition-all hover:scale-105"
      >
  ...
  return (
    <div className="bg-gradient-to-r from-slate-900 to-slate-800 border-b-4 border-orange-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-start gap-6">
          {/* Avatar */}
          <div className="flex-shrink-0">
            <div className="relative">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-4 border-orange-500 shadow-lg">
                <Image
                  src={avatarImage}
                  alt={avatarName}
                  width={96}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded-2xl rounded-tl-none p-4 shadow-lg mb-4 relative">
              <p className="text-gray-800 text-base sm:text-lg leading-relaxed min-h-[60px]">
                {displayedText}
                {isTyping && <span className="inline-block w-2 h-5 bg-orange-500 ml-1 animate-pulse" />}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full transition-all ${currentStep === -1 ? 'bg-orange-500 w-4' : 'bg-slate-600'}`}
                />
                {steps.map((_, index) => (
                  <div
                    key={index}
                    className={`w-2 h-2 rounded-full transition-all ${currentStep === index ? 'bg-orange-500 w-4' : currentStep > index ? 'bg-green-500' : 'bg-slate-600'}`}
                  />
                ))}
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {currentStep === totalSteps - 1 ? (
                  ...
                  <button
                    onClick={handleNext}
                    disabled={isTyping}
                    className="flex items-center gap-1 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition text-sm font-medium disabled:opacity-50"
                  >
