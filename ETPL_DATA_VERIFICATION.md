# ETPL Data Verification - Barber Apprenticeship

**Date:** January 11, 2026  
**Program Location ID:** 10004637  
**Provider Location:** Elevate for Humanity Training Center

---

## Official ETPL Dashboard Data

### Program Information
- **Program Name:** Barber apprenticeship program
- **Classification of Instructional Programs:** 12.0402
- **Provider Location ID:** 10004637

### Program Description (Official)
The Indiana Barbering Apprenticeship Program is a registered, earn-and-learn training program that prepares participants for barber licensure and employment. The program is delivered in two required components: instructional training and on-the-job training (OJT).

**Instructional Training (Milady RISE)**
Participants complete the instructional portion of the program using Milady RISE, an industry-recognized barbering curriculum aligned with Indiana licensing standards. Instruction includes coursework in haircutting, shaving, sanitation and safety, skin and scalp care, customer service, professionalism, and state laws. This portion of the program is delivered through structured online modules and assessments to ensure participants gain the required technical knowledge before entering full-time shop training.

**On-the-Job Training (Licensed Barbershop)**
After completing the instructional training, participants enter a licensed barbershop to complete supervised, paid on-the-job training hours. Apprentices work under the guidance of a licensed barber-mentor and apply classroom instruction in a real-world setting. All training hours are documented and tracked in accordance with Indiana apprenticeship and licensing requirements.

This program is part of a 2,000-hour Registered Apprenticeship and meets Indiana State Board of Barber Examiners, Department of Workforce Development, and U.S. Department of Labor standards. Upon completion, participants are prepared to sit for the state licensure examination and pursue employment as professional barbers.

### Prerequisites
- Must be at least 17 years old
- High school diploma or GED
- Government-issued photo ID
- May require a basic skills assessment or interview

### Program Structure
- **Length of Training:** 15 months
- **Number of Credits:** 260 instructional hours
- **Weekly Clock Hours:** 40.00 Hours
- **Total OJT Hours:** 2,000 hours (Registered Apprenticeship)

### Delivery Format
- **% Offered Online:** 40%
- **% Instructor-Led:** 30%
- **% Lab/Field Work:** 30%
- **% Self-Study:** 40%
- **Times Offered:** Day, Evening, Weekend, Online

### Admissions
- **Percent Admitted:** 10%
- **Offered to General Public:** Yes
- **Part-time Enrollment:** Yes

### Support Services
- **Career Counseling:** Yes
- **Job Placement Offered:** Yes
- **Placement Assistance Available:** Yes

### Financial Information
| Item | Cost | Included in Tuition |
|------|------|---------------------|
| Tuition | $3,990.00 | - |
| Admission Fees | $100.00 | - |
| Books | $150.00 | - |
| Supplies | $300.00 | - |
| Tools | $250.00 | - |
| Labs | $0.00 | - |
| Miscellaneous | $100.00 | - |
| **Total Cost of Program** | **$4,890.00** | - |
| Registered Barber License | $45.00 | No |
| Rise Up (Milady RISE) | $299.00 | Yes |

**Average Cost Per Credit Hour:** $18.81

### Financial Aid Available
- Local Gift Aid (Grants and Scholarships)
- State Financial Aid (Grants and Scholarships)
- Work Study Programs
- **Provider Offers Financial Aid:** No (but accepts WIOA/WRG)

### Application Process
Applications are accepted on a rolling basis. Students may enroll monthly until the cohort reaches capacity. Priority is given to those who complete intake at least 2 weeks prior to the scheduled class start date.

Enrollment confirmations are issued within 5 business days of application review. Orientation and supply pickup will be provided the week before the cohort begins.

### Refund Policy
https://elevateforhumanity.org/refund-policy

### Instructor Qualifications
Program faculty must hold at least one of the following:
- Valid Indiana Barbering license
- Documented industry experience (1+ years in professional setting)

### Industry Collaboration
Yes

---

## Website Updates Made

### ✅ Updated Sections

1. **About the Program**
   - Now uses exact ETPL description
   - Includes Milady RISE details
   - Explains two-component structure
   - Mentions 2,000-hour Registered Apprenticeship

2. **What You'll Learn**
   - Updated to match Milady RISE curriculum
   - Aligned with ETPL course description

3. **Program At-a-Glance**
   - Duration: 15 months (was 12-18 months)
   - Added: 260 instructional hours + 2,000 OJT hours
   - Cost: $4,890 (was $4,950)
   - Format: 40% Online + 60% In-Person
   - Added: Day, Evening, Weekend options

4. **Cost Breakdown**
   - Updated total to $4,890
   - Added itemized breakdown:
     - Tuition: $3,990
     - Admission Fee: $100
     - Books: $150
     - Supplies: $300
     - Tools: $250
     - Miscellaneous: $100
   - Noted Milady RISE ($299) included
   - Noted Barber License Fee ($45) separate

### ⚠️ Performance Metrics Note

**ETPL Dashboard shows:** "Insufficient student exit data. (3 or less)"

This means the program needs to track and report:
- Completion rates
- Employment rates
- Wage gains
- Credential attainment

**Action Required:** Implement robust tracking system for all students to meet ETPL reporting requirements.

---

## Compliance Checklist

### ✅ Website Matches ETPL Data
- [x] Program description matches exactly
- [x] Duration is correct (15 months)
- [x] Cost is correct ($4,890)
- [x] Delivery format is accurate (40% online)
- [x] Prerequisites are listed
- [x] Instructor qualifications mentioned
- [x] Milady RISE curriculum specified
- [x] 2,000-hour apprenticeship noted
- [x] State Board approval mentioned

### ⚠️ Areas Needing Attention
- [ ] Student outcome tracking system
- [ ] Exit data collection (employment, wages)
- [ ] Performance metrics reporting
- [ ] Completion rate tracking
- [ ] Credential attainment tracking

### 📋 Next Steps
1. Implement student tracking system
2. Collect exit data for all graduates
3. Report outcomes to ETPL quarterly
4. Update ETPL dashboard with performance metrics
5. Maintain 10+ student completions for valid data

---

## Contact Information

**ETPL Program Location ID:** 10004637  
**Provider:** Elevate for Humanity Training Center  
**Contact:** elevate4humanityedu@gmail.com  
**Phone:** (317) 314-3757  
**Website:** www.elevateforhumanity.org

---

**Document Version:** 1.0  
**Last Updated:** January 11, 2026  
**Next Review:** Quarterly with ETPL reporting
