...
CREATE POLICY courses_select ON training_courses FOR SELECT
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY courses_insert ON training_courses FOR INSERT
  WITH CHECK (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY courses_update ON training_courses FOR UPDATE
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY courses_delete ON training_courses FOR DELETE
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );
...
CREATE POLICY enrollments_select ON training_enrollments FOR SELECT
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
...
CREATE POLICY enrollments_insert ON training_enrollments FOR INSERT
  WITH CHECK (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY enrollments_update ON training_enrollments FOR UPDATE
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY enrollments_delete ON training_enrollments FOR DELETE
  USING (
    is_super_admin()
  );
...
CREATE POLICY programs_select ON training_programs FOR SELECT
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY programs_insert ON training_programs FOR INSERT
  WITH CHECK (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY programs_update ON training_programs FOR UPDATE
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY programs_delete ON training_programs FOR DELETE
  USING (
    is_super_admin()
  );
...
CREATE POLICY lessons_select ON training_lessons FOR SELECT
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY lessons_insert ON training_lessons FOR INSERT
  WITH CHECK (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY lessons_update ON training_lessons FOR UPDATE
  USING (
    is_super_admin()
    OR tenant_id = get_current_tenant_id()
  );

CREATE POLICY lessons_delete ON training_lessons FOR DELETE
  USING (
    is_super_admin()
  );
