...
DROP POLICY IF EXISTS vendor_payouts_admin_read ON vendor_payouts;
CREATE POLICY vendor_payouts_admin_read ON vendor_payouts
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
  );

ALTER TABLE credential_delivery_queue ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS credential_queue_admin_read ON credential_delivery_queue;
CREATE POLICY credential_queue_admin_read ON credential_delivery_queue
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND role IN ('admin', 'super_admin'))
  );
