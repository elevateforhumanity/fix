...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO volunteer_opportunities (title, description, organization, location, commitment, is_active) VALUES
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO community_groups (name, description, category, member_count, is_active) VALUES
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO certificate_templates (name, description, status) VALUES
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO onboarding_resources (title, description, resource_type, role, order_index, is_active) VALUES
...
-- Support ticket categories (via a sample)$sql$;
EXCEPTION WHEN undefined_table THEN NULL;
WHEN unique_violation THEN NULL;
WHEN not_null_violation THEN NULL;
END $seed$;
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO support_tickets (subject, description, status, priority) VALUES
  ('How do I reset my password?', 'Sample FAQ-style ticket for reference', 'resolved', 'low');

-- Library resources$sql$;
EXCEPTION WHEN undefined_table THEN NULL;
WHEN unique_violation THEN NULL;
WHEN not_null_violation THEN NULL;
END $seed$;
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO library_resources (title, description, resource_type, is_public) VALUES
...
-- Videos (media table has minimal schema — seed into videos table created by migration)$sql$;
EXCEPTION WHEN undefined_table THEN NULL;
WHEN unique_violation THEN NULL;
WHEN not_null_violation THEN NULL;
END $seed$;
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO videos (title, description, category, published, duration_seconds) VALUES
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO leads (first_name, last_name, email, source, status) VALUES
  ('Sample', 'Lead', 'sample@example.com', 'website', 'new');

-- Campaign templates$sql$;
EXCEPTION WHEN undefined_table THEN NULL;
WHEN unique_violation THEN NULL;
WHEN not_null_violation THEN NULL;
END $seed$;
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO campaigns (name, description, type, status, is_active) VALUES
...
DO $seed$ BEGIN
EXECUTE $sql$INSERT INTO job_opportunities (title, description, location, salary_range, job_type, status) VALUES
  ...
  ('IT Support Specialist - Salesforce', 'Help desk and desktop support role', 'Indianapolis, IN (Hybrid)', '$45,000-55,000/yr', 'full-time', 'open')$sql$;
EXCEPTION WHEN undefined_table THEN NULL;
WHEN unique_violation THEN NULL;
WHEN not_null_violation THEN NULL;
END $seed$;