-- Replaces v1 of submit_program_holder_application.
--
-- v2 accepts p_user_id explicitly. The application layer creates the auth user
-- first (so it can generate a magic link), then passes the user ID here.
-- Both `applications` and `program_holders` are written in one transaction.
...
BEGIN
  ...
  INSERT INTO public.applications (
    ...
    support_notes, status, source, user_id
  ) VALUES (
    p_first_name,
    p_last_name,
    lower(trim(p_email)),
  ...
  -- 2. Upsert program_holders row linked to the user
  ...
  -- 3. Link profile → program_holders
