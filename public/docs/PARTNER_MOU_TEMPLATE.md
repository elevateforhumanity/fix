# MEMORANDUM OF UNDERSTANDING (MOU)
## Program Holder Partnership Agreement

**Between:**

**Elevate for Humanity**
Indianapolis, Indiana
("Elevate" or "EFH")

**AND**

**[PARTNER ORGANIZATION NAME]**
[Address]
[City, State ZIP]
("Program Holder" or "Partner")

**Effective Date:** _______________

---

## I. PURPOSE

This Memorandum of Understanding (MOU) establishes a partnership between Elevate for Humanity and [Partner Name] to provide quality workforce training and career development services to students in the following program(s):

**Program(s) Covered:**
- [ ] HVAC Technician Training
- [ ] Medical Assistant Training
- [ ] Barber Apprenticeship
- [ ] CDL Training
- [ ] Cosmetology/Esthetician
- [ ] Business Start-Up
- [ ] Tax Preparation
- [ ] Direct Support Professional
- [ ] Other: _______________________

---

## II. PARTNERSHIP SCOPE

### A. Elevate for Humanity Responsibilities

Elevate for Humanity agrees to:

1. **Student Recruitment & Referrals**
   - Market programs to eligible students
   - Pre-screen applicants for basic eligibility
   - Coordinate enrollment process
   - Provide student onboarding support

2. **Funding Coordination**
   - Assist with WIOA, WRG, JAG, and other funding applications
   - Coordinate with workforce boards and funding agencies
   - Process tuition payments according to agreed terms
   - Maintain funding compliance documentation

3. **Support Services**
   - Provide student support and case management
   - Offer digital learning platform access
   - Supply marketing and promotional materials
   - Facilitate employer connections for job placement

4. **Compliance & Reporting**
   - Maintain centralized student records
   - Coordinate required reporting to funding agencies
   - Provide compliance templates and guidance
   - Conduct periodic quality assurance reviews

### B. Program Holder Responsibilities

Program Holder agrees to:

1. **Program Delivery**
   - Provide quality instruction per approved curriculum
   - Maintain qualified, credentialed instructors
   - Ensure safe, accessible training facilities
   - Follow all applicable state and federal regulations

2. **Student Services**
   - Accept referred students meeting program prerequisites
   - Track and report student attendance
   - Communicate student progress and concerns
   - Provide reasonable accommodations as required by law
   - Issue certificates/credentials upon successful completion

3. **Compliance & Documentation**
   - Maintain current licenses, approvals, and insurance
   - Submit required reports on time
   - Notify Elevate of any compliance issues immediately
   - Participate in quality assurance reviews
   - Maintain student records per FERPA requirements

4. **Communication**
   - Respond to Elevate inquiries within 2 business days
   - Notify Elevate of schedule changes at least 2 weeks in advance
   - Report student withdrawals within 24 hours
   - Participate in monthly partner check-in calls

---

## III. FINANCIAL TERMS

### A. Tuition & Fees

**Program Tuition:** $__________ per student
**Payment Schedule:**
- [ ] Upfront (upon enrollment)
- [ ] 50% at start / 50% at midpoint
- [ ] Monthly installments
- [ ] Upon completion
- [ ] Other: _______________________

**Additional Fees:**
- Materials/Supplies: $__________
- Testing/Certification: $__________
- Other: $__________

### B. Payment Processing

1. Elevate will coordinate payment from funding sources or students
2. Payments to Program Holder will be processed within _____ days of invoice receipt
3. Program Holder will submit invoices with required documentation
4. Refund policy for student withdrawals: _______________________

### C. Revenue Sharing (if applicable)

- [ ] Not applicable
- [ ] Elevate retains ____% for administrative services
- [ ] Program Holder receives ____% of tuition
- [ ] Other arrangement: _______________________

---

## IV. PROGRAM DETAILS

### A. Program Information

**Program Name:** _______________________
**Duration:** _______ weeks/months
**Total Hours:** _______ hours
**Schedule:** _______________________
**Location:** _______________________
**Capacity:** _______ students per cohort
**Prerequisites:** _______________________

### B. Curriculum & Standards

- Program Holder will follow curriculum approved by: _______________________
- Industry certifications offered: _______________________
- Completion requirements: _______________________
- Attendance policy: _______________________

---

## V. STUDENT ENROLLMENT & CAPACITY

### A. Enrollment Process

1. Elevate refers qualified students to Program Holder
2. Program Holder reviews applications within _____ business days
3. Program Holder conducts intake/orientation
4. Enrollment confirmed and start date scheduled

### B. Capacity Commitment

Program Holder commits to accepting:
- Minimum: _____ students per cohort
- Maximum: _____ students per cohort
- Annual capacity: _____ students

### C. Student Selection

Program Holder may decline students only for:
- Failure to meet stated prerequisites
- Lack of capacity
- Documented safety concerns
- Other legitimate, non-discriminatory reasons

---

## VI. REPORTING REQUIREMENTS

### A. Monthly Reports (Due 5th of each month)

- Student attendance records
- Progress updates
- Completion notifications
- Any issues or concerns

### B. Quarterly Reports

- Outcome data (completions, job placements)
- Student satisfaction surveys
- Program improvements or changes

### C. Ad-Hoc Reporting

- Student withdrawals (within 24 hours)
- Serious incidents (immediately)
- Schedule changes (2 weeks advance notice)

---

## VII. QUALITY ASSURANCE

### A. Standards

Program Holder agrees to maintain:
- Current state licenses and approvals
- Qualified instructors with appropriate credentials
- Safe, accessible facilities meeting ADA requirements
- Adequate liability insurance ($_______ minimum)
- Compliance with all applicable regulations

### B. Reviews

- Elevate may conduct periodic site visits
- Student satisfaction surveys will be administered
- Annual partnership review and renewal
- Corrective action plans if standards not met

---

## VIII. COMPLIANCE & LEGAL

### A. Regulatory Compliance

Both parties agree to comply with:
- WIOA regulations (if applicable)
- State workforce development requirements
- FERPA (student privacy)
- ADA (accessibility)
- Title VI (non-discrimination)
- All applicable federal, state, and local laws

### B. Non-Discrimination

Neither party shall discriminate based on:
- Race, color, national origin
- Sex, gender identity, sexual orientation
- Religion
- Age
- Disability
- Veteran status
- Any other protected class

### C. Confidentiality

Both parties agree to:
- Protect student personally identifiable information (PII)
- Comply with FERPA requirements
- Use data only for agreed purposes
- Maintain secure data storage and transmission

---

## IX. INSURANCE & LIABILITY

### A. Required Insurance

Program Holder shall maintain:
- General Liability: $_______ minimum
- Professional Liability: $_______ minimum (if applicable)
- Workers' Compensation (as required by law)
- Property Insurance (if applicable)

### B. Indemnification

Each party agrees to indemnify and hold harmless the other party from claims arising from their own negligence or misconduct.

---

## X. INTELLECTUAL PROPERTY

### A. Ownership

- Program Holder retains ownership of proprietary curriculum
- Elevate retains ownership of platform, branding, and materials
- Student work products belong to students

### B. Licensing

- Program Holder grants Elevate license to market programs
- Elevate grants Program Holder license to use platform
- Co-branded materials require mutual approval

---

## XI. TERM & TERMINATION

### A. Term

This MOU is effective from _____________ and continues for:
- [ ] One (1) year, with automatic renewal
- [ ] Two (2) years
- [ ] Ongoing until terminated
- [ ] Other: _______________________

### B. Termination

Either party may terminate this MOU:
- With 60 days written notice
- Immediately for material breach
- Immediately for loss of required licenses/approvals
- Immediately for fraud or misrepresentation

### C. Wind-Down

Upon termination:
- Current students will be allowed to complete programs
- Final payments will be processed
- All confidential information will be returned or destroyed
- Parties will cooperate on smooth transition

---

## XII. DISPUTE RESOLUTION

### A. Process

1. Good faith negotiation between parties
2. Mediation (if negotiation fails)
3. Arbitration (if mediation fails)
4. Litigation (last resort)

### B. Governing Law

This MOU shall be governed by the laws of the State of Indiana.

---

## XIII. GENERAL PROVISIONS

### A. Entire Agreement

This MOU constitutes the entire agreement between parties and supersedes all prior agreements.

### B. Amendments

Amendments must be in writing and signed by both parties.

### C. Independent Contractors

Parties are independent contractors, not employees or agents of each other.

### D. Force Majeure

Neither party liable for delays due to circumstances beyond reasonable control.

### E. Notices

All notices shall be sent to:

**Elevate for Humanity:**
Elizabeth Greene, Founder
[Address]
Email: elizabeth@www.elevateforhumanity.org

**Program Holder:**
[Contact Name, Title]
[Address]
Email: _______________________

---

## XIV. SIGNATURES

By signing below, both parties agree to the terms of this MOU:

**ELEVATE FOR HUMANITY**

Signature: _______________________
Name: Elizabeth Greene
Title: Founder
Date: _______________________


**[PARTNER ORGANIZATION NAME]**

Signature: _______________________
Name: _______________________
Title: _______________________
Date: _______________________

---

## ATTACHMENTS

- [ ] Attachment A: Program Curriculum Outline
- [ ] Attachment B: Fee Schedule
- [ ] Attachment C: Reporting Templates
- [ ] Attachment D: Insurance Certificates
- [ ] Attachment E: Licenses and Approvals
- [ ] Attachment F: W-9 Form

---

**For questions about this MOU, contact:**
Elizabeth Greene
Founder, Elevate for Humanity
elizabeth@www.elevateforhumanity.org
(317) 555-0100
