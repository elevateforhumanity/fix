# Beauty & Career Educator Training
## Program Syllabus

**Provider:** Elevate for Humanity  
**ETPL Provider ID:** 10000949  
**Program Code:** BEAUTY-EDU  
**Duration:** 12-16 Weeks (320-480 Hours)  
**Delivery Method:** Hybrid (Classroom + Teaching Practicum)  
**Funding:** WIOA • WRG  

---

## Program Overview

Become a licensed beauty instructor! This program prepares experienced cosmetologists and barbers to teach the next generation. Learn curriculum development, classroom management, and state licensing requirements. Perfect for those with 3+ years industry experience.

---

## Learning Objectives

1. Develop lesson plans and curriculum for beauty programs
2. Apply adult learning principles and teaching methods
3. Manage classroom and lab environments effectively
4. Assess student performance and provide feedback
5. Meet Indiana State Board instructor requirements
6. Demonstrate professional teaching practices
7. Integrate technology in beauty education
8. Prepare students for state licensing exams

---

## Course Outline

### Module 1: Foundations of Adult Education (40 hours)
- Adult learning theory
- Learning styles and preferences
- Instructional design basics
- Curriculum development
- Lesson planning
- Educational psychology
- Diversity and inclusion in education

### Module 2: Teaching Methods & Strategies (60 hours)
- Lecture and demonstration techniques
- Hands-on instruction methods
- Group activities and projects
- Technology integration
- Visual aids and multimedia
- Differentiated instruction
- Flipped classroom approaches

### Module 3: Classroom Management (40 hours)
- Creating positive learning environment
- Student motivation and engagement
- Behavior management strategies
- Conflict resolution
- Time management
- Attendance and punctuality policies
- Professional boundaries

### Module 4: Assessment & Evaluation (40 hours)
- Types of assessments (formative, summative)
- Written test development
- Practical skills evaluation
- Rubrics and grading criteria
- Providing constructive feedback
- Progress tracking and documentation
- State Board exam preparation

### Module 5: Beauty Industry Curriculum (60 hours)
- Cosmetology curriculum standards
- Barbering curriculum standards
- Esthetics curriculum standards
- Nail technology curriculum
- State Board requirements
- Industry trends and updates
- Safety and sanitation instruction

### Module 6: Lab & Clinical Instruction (40 hours)
- Demonstrating technical skills
- Supervising student practice
- Correcting technique errors
- Safety supervision
- Client service instruction
- Salon floor management
- Quality control

### Module 7: Professional Development (20 hours)
- Indiana State Board regulations
- Instructor licensing requirements
- Continuing education
- Professional organizations
- Career advancement
- Ethics and professionalism
- Industry networking

### Module 8: Teaching Practicum (80-160 hours)
- Supervised teaching in beauty school
- Lesson plan implementation
- Student instruction and supervision
- Classroom management practice
- Assessment administration
- Mentor feedback and coaching
- Professional portfolio development

---

## Assessment Methods

- Lesson plan development and presentation
- Teaching demonstrations (peer and student)
- Classroom management scenarios
- Written exams (70% passing)
- Practicum evaluation by mentor instructor
- Final comprehensive teaching demonstration
- Professional portfolio

---

## Required Materials

- Teaching methods textbook (provided)
- Curriculum development workbook (provided)
- Professional attire for teaching
- Technology tools (laptop, projector access)
- State Board instructor application materials

---

## Instructor Qualifications

- Licensed cosmetology or barber instructor (Indiana)
- Minimum 5 years teaching experience
- Master educator certification (preferred)
- Current industry knowledge
- Adult education credential

---

## Prerequisites

Students must have:
- Current Indiana cosmetology or barber license
- Minimum 3 years professional experience
- High school diploma or equivalent
- Good standing with State Board
- Background check clearance

---

## Attendance Policy

- Required attendance: 95% for classroom, 100% for practicum
- Teaching practicum hours must be completed in full
- Professional conduct required at all times

---

## Career Outcomes

- Cosmetology Instructor
- Barber Instructor
- Esthetics Instructor
- Nail Technology Instructor
- Beauty School Director
- Curriculum Developer
- Educational Consultant

**Average Starting Wage:** $18-$25/hour  
**Experienced Instructor:** $25-$35/hour  
**School Director:** $40,000-$60,000+/year

---

## Indiana Instructor Licensure

To obtain Indiana beauty instructor license, candidates must:
1. Hold current cosmetology or barber license
2. Have 3+ years professional experience
3. Complete approved instructor training program (this program)
4. Be at least 21 years old
5. Pass State Board instructor exam
6. Submit application and fees
7. Maintain continuing education

---

## Program Completion Requirements

- Complete all instructional hours
- Pass all written exams (70%)
- Successfully complete teaching practicum
- Demonstrate effective teaching methods
- Submit professional teaching portfolio
- Receive satisfactory mentor evaluations
- Meet State Board instructor requirements

---

## Support Services

- Career coaching and job placement
- State Board exam preparation
- Instructor license application assistance
- Resume and interview preparation
- Continuing education opportunities
- Professional development workshops
- Alumni network

---

## Contact Information

**Elevate for Humanity**  
Website: elevateforhumanity.institute  
Email: elevate4humanityedu@gmail.com  
Phone: (317) 314-3757

**Indiana State Board of Cosmetology & Barber Examiners:**  
Phone: (317) 234-8320  
Website: www.in.gov/pla

**Indiana DWD/WorkOne Funding:**  
Phone: 1-800-891-6499  
WorkOne Indianapolis: (317) 684-2400

---

**Last Updated:** November 2024  
**ETPL Approved:** Yes  
**State Board Approved:** Yes
