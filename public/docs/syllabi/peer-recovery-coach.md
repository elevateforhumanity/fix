# Certified Peer Recovery Coach (CPRC)
## Program Syllabus

**Provider:** Elevate for Humanity  
**ETPL Provider ID:** 10000949  
**Program Code:** CPRC-CERT  
**Duration:** 45 Days (180 Hours)  
**Delivery Method:** Hybrid (Classroom + Practicum)  
**Funding:** WIOA • JRI  

---

## Program Overview

Support individuals in addiction recovery! Become a Certified Peer Recovery Coach and help others navigate their recovery journey. Learn motivational interviewing, relapse prevention, and community resources. Justice-involved individuals welcome.

---

## Learning Objectives

1. Apply peer support principles in recovery settings
2. Use motivational interviewing techniques
3. Develop recovery plans with clients
4. Connect clients to community resources
5. Recognize and respond to relapse warning signs
6. Maintain professional boundaries and ethics
7. Document peer support services accurately
8. Advocate for individuals in recovery

---

## Course Outline

### Module 1: Foundations of Peer Support (30 hours)
- History and philosophy of peer support
- Recovery-oriented systems of care
- Peer support core competencies
- Ethics and professional boundaries
- Self-care and wellness
- Cultural competency and diversity

### Module 2: Addiction & Recovery (30 hours)
- Substance use disorders overview
- Brain science of addiction
- Stages of change model
- Recovery pathways and models
- Co-occurring disorders
- Trauma-informed care

### Module 3: Peer Support Skills (40 hours)
- Active listening and empathy
- Motivational interviewing basics
- Goal setting and action planning
- Problem-solving strategies
- Crisis intervention basics
- Conflict resolution

### Module 4: Relapse Prevention (20 hours)
- Relapse warning signs
- Triggers and coping strategies
- Developing relapse prevention plans
- Supporting clients through setbacks
- Building recovery capital
- Long-term recovery maintenance

### Module 5: Community Resources & Navigation (20 hours)
- Treatment and recovery services
- Housing and employment resources
- Healthcare and benefits navigation
- Legal system navigation
- Family and social support
- Recovery community organizations

### Module 6: Documentation & Professional Practice (20 hours)
- HIPAA and confidentiality
- Progress notes and documentation
- Supervision and consultation
- Working within treatment teams
- Professional development
- Certification requirements

### Module 7: Supervised Practicum (20 hours)
- Observation of peer support services
- Supervised client interactions
- Case study discussions
- Skills practice and feedback
- Professional portfolio development

---

## Assessment Methods

- Module quizzes (70% passing)
- Skills demonstrations (motivational interviewing, crisis response)
- Case study presentations
- Practicum evaluation
- Final comprehensive exam (75% passing)
- Professional portfolio

---

## Certification Preparation

Program prepares students for:
- **Indiana Certified Peer Recovery Coach (CPRC)** - Indiana Division of Mental Health and Addiction
- **National Certified Peer Recovery Support Specialist (NCPRSS)** - IC&RC

---

## Required Materials

- Textbook: "Peer Support Specialist Training Manual" (provided)
- Workbook and case studies (provided)
- Professional attire for practicum
- Background check (required)
- Drug screening (may be required by practicum site)

---

## Instructor Qualifications

- Certified Peer Recovery Coach or equivalent
- Minimum 3 years recovery experience
- Experience in peer support or behavioral health
- Training in adult education or facilitation

---

## Attendance Policy

- Required attendance: 95% (171 of 180 hours)
- Practicum hours must be completed in full
- Absences may delay certification eligibility

---

## Career Outcomes

- Certified Peer Recovery Coach
- Peer Support Specialist
- Recovery Coach
- Community Health Worker
- Case Management Assistant
- Outreach Worker

**Average Starting Wage:** $14-$18/hour  
**Job Placement Rate:** 85% within 90 days

---

## Program Completion Requirements

- Complete all 180 hours
- Pass all module quizzes (70%)
- Demonstrate competency in peer support skills
- Complete supervised practicum
- Pass final exam (75%)
- Submit professional portfolio
- Meet Indiana CPRC eligibility requirements

---

## Indiana CPRC Certification Requirements

To apply for state certification, candidates must:
1. Be at least 18 years old
2. Have high school diploma or equivalent
3. Have personal recovery experience (1+ year)
4. Complete approved training program (this program)
5. Pass background check
6. Submit application to Indiana DMHA
7. Maintain continuing education

---

## Support Services

- Career coaching and job placement
- Resume and interview preparation
- Certification application assistance
- Peer mentorship and support groups
- Continuing education opportunities
- Alumni network

---

## Contact Information

**Elevate for Humanity**  
Website: elevateforhumanity.institute  
Email: elevate4humanityedu@gmail.com  
Phone: (317) 314-3757

**Indiana Division of Mental Health and Addiction:**  
Website: www.in.gov/dmha  
Phone: (317) 232-7800

---

**Last Updated:** November 2024  
**ETPL Approved:** Yes  
**Indiana DMHA Approved Training:** Yes
