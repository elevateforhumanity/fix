# Pharmacy Technician
## Program Syllabus

**Provider:** Elevate for Humanity  
**ETPL Provider ID:** 10000949  
**Program Code:** PHARM-TECH  
**Duration:** 12-16 Weeks (320-480 Hours)  
**Delivery Method:** Hybrid (Online + Externship)  
**Funding:** WRG • WIOA • Workforce Grants  

---

## Program Overview

Enter the growing pharmacy field in 12-16 weeks! Learn medication dispensing, prescription processing, inventory management, insurance billing, and patient service. Program includes externship at retail or hospital pharmacies. Graduate ready for national PTCB certification and employment at CVS, Walgreens, hospitals, and clinics.

---

## Learning Objectives

1. Process and fill prescriptions accurately
2. Perform pharmacy calculations and conversions
3. Maintain pharmacy inventory and ordering systems
4. Process insurance claims and prior authorizations
5. Provide customer service and patient education
6. Apply pharmacy law and regulations
7. Compound medications under pharmacist supervision
8. Use pharmacy information systems

---

## Course Outline

### Module 1: Introduction to Pharmacy Practice (40 hours)
- Role of pharmacy technician
- Healthcare delivery systems
- Pharmacy settings (retail, hospital, specialty)
- Professional ethics and conduct
- HIPAA and patient confidentiality
- Pharmacy law and regulations

### Module 2: Pharmacology & Medication Safety (60 hours)
- Drug classifications and therapeutic uses
- Brand and generic drug names
- Routes of administration
- Drug interactions and contraindications
- Medication errors and prevention
- High-alert medications
- Look-alike/sound-alike drugs

### Module 3: Pharmacy Calculations (40 hours)
- Measurement systems (metric, household, apothecary)
- Ratio and proportion
- Percentage calculations
- Dosage calculations
- Dilutions and concentrations
- IV flow rates
- Pediatric and geriatric dosing

### Module 4: Prescription Processing (60 hours)
- Reading and interpreting prescriptions
- Prescription data entry
- Drug utilization review (DUR)
- Label generation
- Medication selection and counting
- Prescription verification process
- Refill procedures

### Module 5: Pharmacy Information Systems (30 hours)
- Pharmacy software navigation
- Patient profile management
- Prescription processing workflow
- Inventory management systems
- Electronic prescribing (e-prescribing)
- Medication therapy management (MTM)

### Module 6: Insurance & Billing (40 hours)
- Insurance plan types
- Prescription insurance processing
- Prior authorizations
- Rejected claims troubleshooting
- Medicare Part D
- Medicaid programs
- Discount programs and coupons

### Module 7: Sterile & Non-Sterile Compounding (50 hours)
- USP <795> and <797> standards
- Aseptic technique
- IV admixture preparation
- Compounding equipment and supplies
- Quality assurance
- Documentation and labeling
- Hazardous drug handling

### Module 8: Inventory Management (30 hours)
- Ordering and receiving medications
- Stock rotation and expiration dating
- Controlled substance regulations
- DEA requirements and documentation
- Medication recalls
- Automated dispensing systems

### Module 9: Customer Service & Communication (20 hours)
- Professional communication
- Patient counseling support
- Telephone etiquette
- Handling difficult situations
- Cultural competency
- Teamwork and collaboration

### Module 10: Externship (120-200 hours)
- Supervised practice in retail or hospital pharmacy
- Prescription processing
- Patient interaction
- Inventory management
- Insurance billing
- Professional conduct
- Preceptor evaluation

---

## Assessment Methods

- Module exams (70% passing)
- Pharmacy calculations assessments (90% accuracy required)
- Skills demonstrations (compounding, prescription processing)
- Externship evaluation
- Mock PTCB exam
- Final comprehensive exam (75% passing)

---

## Required Materials

- Pharmacy technician textbook (provided)
- Drug reference guide (provided)
- Calculator (provided)
- Lab coat and closed-toe shoes
- Background check and drug screening
- Immunization records (TB, Hepatitis B, MMR, Varicella, Flu)

---

## Instructor Qualifications

- Certified Pharmacy Technician (CPhT) or Registered Pharmacist (RPh/PharmD)
- Minimum 3 years pharmacy practice experience
- Teaching experience or adult education background
- Current knowledge of pharmacy law and practice

---

## Externship Requirements

Students complete supervised practice at approved pharmacies:
- Retail pharmacies (CVS, Walgreens, Kroger, etc.)
- Hospital pharmacies
- Specialty pharmacies
- Long-term care pharmacies

**Minimum Hours:** 120-200 hours depending on program track

---

## Attendance Policy

- Required attendance: 90% for didactic, 100% for externship
- Externship absences must be made up
- Professional behavior required at all times

---

## Career Outcomes

- Pharmacy Technician (retail)
- Hospital Pharmacy Technician
- Specialty Pharmacy Technician
- Long-Term Care Pharmacy Technician
- Mail-Order Pharmacy Technician
- Pharmacy Inventory Specialist

**Average Starting Wage:** $15-$19/hour  
**Experienced Wage:** $19-$24/hour  
**Job Placement Rate:** 90% within 60 days

---

## Certifications Earned

Students are prepared to sit for:
1. **Pharmacy Technician Certification Board (PTCB)** - CPhT
2. **National Healthcareer Association (NHA)** - ExCPT
3. **CPR/BLS** - American Heart Association

---

## State Registration

Indiana pharmacy technicians must:
- Register with Indiana Board of Pharmacy
- Pass national certification exam (PTCB or NHA)
- Complete background check
- Maintain continuing education (20 hours every 2 years)

---

## Program Completion Requirements

- Complete all didactic hours
- Pass all module exams (70%)
- Achieve 90% accuracy on pharmacy calculations
- Complete externship hours
- Pass final exam (75%)
- Pass mock PTCB exam (650+ score)
- Maintain professional conduct

---

## Support Services

- Career coaching and job placement
- Resume and interview preparation
- PTCB exam preparation and study materials
- Uniform assistance
- Transportation support (if eligible)
- State registration assistance
- Continuing education opportunities

---

## Contact Information

**Elevate for Humanity**  
Website: elevateforhumanity.institute  
Email: elevate4humanityedu@gmail.com  
Phone: (317) 314-3757

**Indiana DWD/WorkOne Funding:**  
Phone: 1-800-891-6499  
WorkOne Indianapolis: (317) 314-3757

---

**Last Updated:** November 2024  
**ETPL Approved:** Yes
