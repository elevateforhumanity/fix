# Building Maintenance Technician
## Program Syllabus

**Provider:** Elevate for Humanity  
**ETPL Provider ID:** 10000949  
**Program Code:** BLDG-MAINT  
**Duration:** 4-9 Months (320-720 Hours)  
**Delivery Method:** Hands-On Training + Field Experience  
**Funding:** Workforce Grants • Apprenticeship • WIOA  

---

## Program Overview

Become the go-to person who keeps buildings running! Learn plumbing basics, electrical repairs, HVAC maintenance, carpentry, and safety systems through hands-on training. Prepare for steady employment with property management companies, schools, hospitals, and commercial buildings.

---

## Learning Objectives

1. Perform routine maintenance on building systems
2. Troubleshoot and repair plumbing issues
3. Conduct basic electrical repairs safely
4. Maintain HVAC systems and equipment
5. Execute carpentry and drywall repairs
6. Operate power tools and equipment safely
7. Respond to maintenance requests efficiently
8. Document work orders and maintenance activities

---

## Course Outline

### Module 1: Safety & Tools (40 hours)
- OSHA 10-Hour Construction Safety
- Personal protective equipment
- Hand and power tool safety
- Ladder and scaffold safety
- Lockout/tagout procedures
- Hazard communication

### Module 2: Plumbing Basics (80 hours)
- Plumbing systems overview
- Pipe types and fittings
- Drain cleaning and unclogging
- Faucet and toilet repairs
- Water heater maintenance
- Leak detection and repair

### Module 3: Electrical Fundamentals (80 hours)
- Electrical safety
- Basic circuits and wiring
- Outlet and switch replacement
- Lighting fixture installation
- Troubleshooting electrical problems
- Reading electrical diagrams

### Module 4: HVAC Maintenance (60 hours)
- HVAC system components
- Filter replacement
- Thermostat operation
- Basic troubleshooting
- Preventive maintenance
- Energy efficiency

### Module 5: Carpentry & Drywall (60 hours)
- Measuring and layout
- Door and window repairs
- Drywall patching and repair
- Painting and finishing
- Cabinet and hardware installation
- Flooring basics

### Module 6: Building Systems (40 hours)
- Fire alarm and sprinkler systems
- Security systems basics
- Elevator and escalator awareness
- Boiler and chiller basics
- Building automation systems
- Energy management

### Module 7: Preventive Maintenance (40 hours)
- Maintenance scheduling
- Equipment inspection procedures
- Lubrication and cleaning
- Record keeping and documentation
- Inventory management
- Work order systems

### Module 8: Customer Service & Communication (20 hours)
- Professional communication
- Responding to tenant requests
- Emergency response procedures
- Time management
- Teamwork and collaboration

### Module 9: Field Training (200-400 hours)
- Supervised work at facility
- Real maintenance requests
- Emergency response
- Preventive maintenance rounds
- Equipment operation
- Professional conduct

---

## Assessment Methods

- Module exams (70% passing)
- Hands-on skills demonstrations
- OSHA 10 certification
- Field training evaluation
- Final comprehensive exam (75%)

---

## Certifications Earned

1. **OSHA 10-Hour Construction Safety**
2. **EPA Section 608 Type I** (HVAC refrigerant handling)
3. **Forklift Operator Certification** (if applicable)
4. **CPR/First Aid**

---

## Career Outcomes

- Building Maintenance Technician
- Facilities Maintenance Worker
- Property Maintenance Specialist
- Apartment Maintenance Technician
- School/Hospital Maintenance Worker
- Maintenance Supervisor (with experience)

**Average Starting Wage:** $16-$22/hour  
**Experienced Wage:** $22-$30/hour

---

## Program Completion Requirements

- Complete all instructional hours
- Pass all module exams (70%)
- Demonstrate hands-on competency
- Complete field training hours
- Pass final exam (75%)
- Earn OSHA 10 certification

---

## Contact Information

**Elevate for Humanity**  
Website: elevateforhumanity.institute  
Phone: (317) 314-3757

---

**Last Updated:** November 2024  
**ETPL Approved:** Yes
