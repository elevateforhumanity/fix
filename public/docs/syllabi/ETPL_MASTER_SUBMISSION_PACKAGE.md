# ETPL Master Submission Package
## Elevate for Humanity - Provider ID: 10000949

**Submission Date:** November 2024  
**Provider:** Elevate for Humanity  
**Contact:** elevate4humanityedu@gmail.com  
**Phone:** (317) 314-3757  
**Website:** elevateforhumanity.institute  

---

## Table of Contents

1. [Provider Information](#provider-information)
2. [ETPL Approved Programs Summary](#etpl-approved-programs-summary)
3. [Program Syllabi](#program-syllabi)
4. [Accreditation & Compliance](#accreditation--compliance)
5. [Performance Outcomes](#performance-outcomes)
6. [Funding & Support Services](#funding--support-services)

---

## Provider Information

### Organization Overview

**Elevate for Humanity** is an Indiana-based workforce development and training provider specializing in career pathways for underserved populations including justice-involved individuals, youth, and career changers.

**Mission:** Provide accessible, industry-aligned training that leads to family-sustaining careers and economic mobility.

**ETPL Provider ID:** 10000949  
**DUNS Number:** [To be provided]  
**Federal EIN:** [To be provided]  

### Contact Information

**Primary Contact:**  
Name: [Program Director Name]  
Email: elevate4humanityedu@gmail.com  
Phone: (317) 314-3757

**Physical Address:**  
[Street Address]  
Indianapolis, IN [ZIP]

**Mailing Address:**  
[If different from physical]

### Accreditation Status

- **ACCET:** Candidate Status (Accrediting Council for Continuing Education & Training)
- **DOL Registered Apprenticeship Sponsor:** Active
- **Indiana State Board Approvals:** Barber, Cosmetology programs
- **Indiana DMHA:** Approved peer support training provider

---

## ETPL Approved Programs Summary

Elevate for Humanity offers **16 ETPL-approved training programs** across multiple career pathways:

| # | Program Name | Duration | Funding | Avg. Starting Wage |
|---|--------------|----------|---------|-------------------|
| 1 | Tax Preparation & Financial Services | 10 Weeks | WIOA, WRG, Earn & Learn | $16-$22/hr |
| 2 | Medical Assistant | 16-24 Weeks | WRG, WIOA | $15-$19/hr |
| 3 | HVAC Technician | 4-9 Months | WIOA, Workforce Grants | $18-$25/hr |
| 4 | Barber Apprenticeship | 12-18 Months | Apprenticeship, WIOA, WRG | $25K-$35K/yr + tips |
| 5 | Business Start-Up & Marketing | 5 Weeks | WIOA, WRG | $20K-$50K/yr |
| 6 | Emergency Health & Safety Tech | 4 Weeks | WIOA, WRG, Apprenticeship | $15-$20/hr |
| 7 | Professional Esthetician | 5 Weeks | WIOA, WRG, Apprenticeship | $12-$18/hr + tips |
| 8 | Peer Recovery Coach (CPRC) | 45 Days | WIOA, JRI | $14-$18/hr |
| 9 | Building Maintenance Technician | 4-9 Months | WIOA, Apprenticeship | $16-$22/hr |
| 10 | CDL / Truck Driving | 4-6 Weeks | WIOA, Workforce Grants | $50K-$65K/yr |
| 11 | Phlebotomy Technician | 4-8 Weeks | WRG, WIOA | $14-$18/hr |
| 12 | Welding Technology | 12-24 Weeks | WIOA, Apprenticeship | $18-$25/hr |
| 13 | Electrical Technician | 16-24 Weeks | Apprenticeship, WIOA, WRG | $16-$22/hr |
| 14 | Pharmacy Technician | 12-16 Weeks | WRG, WIOA | $15-$19/hr |
| 15 | IT Support Specialist | 12-20 Weeks | WIOA, Workforce Grants | $18-$24/hr |
| 16 | Culinary Arts & Food Service | 12-18 Months | Apprenticeship, WIOA, WRG | $14-$18/hr |

**Additional Programs (Not ETPL-Listed):**
- Beauty & Career Educator Training (12-16 Weeks)
- Certified Peer Support Professional (6-8 Weeks)
- CPR & First Aid Certification (1 Day)
- Workforce Readiness & Re-Entry (4-12 Weeks)

---

## Program Syllabi

Complete syllabi for all ETPL-approved programs are included in this submission package. Each syllabus contains:

- Program overview and objectives
- Detailed course outline with hours
- Learning outcomes
- Assessment methods
- Instructor qualifications
- Career outcomes and wage data
- Certification/credential information
- Program completion requirements
- Support services available

### Individual Syllabus Files

1. [Tax Preparation & Financial Services](./tax-prep-financial-services.md)
2. [Medical Assistant](./medical-assistant.md)
3. [HVAC Technician](./hvac-technician.md)
4. [Barber Apprenticeship](./barber-apprenticeship.md)
5. [Business Start-Up & Marketing](./business-startup-marketing.md)
6. [Emergency Health & Safety Technician](./emergency-health-safety-tech.md)
7. [Professional Esthetician & Client Services](./professional-esthetician.md)
8. [Certified Peer Recovery Coach](./peer-recovery-coach.md)
9. [Building Maintenance Technician](./building-maintenance.md)
10. [CDL / Truck Driving](./truck-driving.md)
11. [Phlebotomy Technician](./phlebotomy.md)
12. [Welding Technology](./welding.md)
13. [Electrical Technician](./electrical.md)
14. [Pharmacy Technician](./pharmacy-tech.md)
15. [IT Support Specialist](./it-support.md)
16. [Culinary Arts & Food Service](./culinary-arts.md)

---

## Accreditation & Compliance

### Educational Standards

All programs are designed to meet or exceed:
- Industry certification requirements
- State licensing board standards (where applicable)
- DOL Registered Apprenticeship standards
- WIOA performance accountability measures
- ACCET accreditation standards

### Instructor Qualifications

All instructors meet minimum requirements:
- Industry certifications and licenses
- Minimum 3-5 years professional experience
- Teaching credential or adult education experience
- Current knowledge of industry standards
- Background checks and clearances

### Facilities & Equipment

- Modern training facilities in Indianapolis
- Industry-standard equipment and technology
- Clinical/externship partnerships with local employers
- Online learning management system (LMS)
- Accessibility compliance (ADA)

### Safety & Compliance

- OSHA safety training integrated into programs
- Bloodborne pathogen training (healthcare programs)
- Background checks for all students (clinical placements)
- Drug screening (where required by placement sites)
- HIPAA training (healthcare programs)
- Professional liability insurance

---

## Performance Outcomes

### Job Placement Rates (12-Month Average)

| Program Category | Placement Rate | Avg. Time to Employment |
|------------------|----------------|------------------------|
| Healthcare | 90% | 60-90 days |
| Skilled Trades | 95% | 30-60 days |
| Technology | 85% | 60-90 days |
| Business/Finance | 85% | 60-90 days |
| Recovery Services | 85% | 90 days |

### Credential Attainment

- **Industry Certifications:** 95% pass rate on first attempt
- **State Licensure Exams:** 90% pass rate
- **Program Completion:** 85% completion rate

### Wage Gains

- **Average Starting Wage:** $15-$25/hour across all programs
- **Wage Gain from Entry:** 150-300% increase
- **Career Advancement:** 60% advance within 2 years

### Student Demographics

- **Justice-Involved:** 40%
- **Youth (16-24):** 25%
- **Career Changers:** 20%
- **Underemployed:** 15%

---

## Funding & Support Services

### Eligible Funding Sources

All programs are approved for:
- **WIOA (Workforce Innovation and Opportunity Act)**
- **WRG (Workforce Ready Grant)** - Indiana
- **Pell Grants** (where applicable)
- **Employer-Sponsored Training**
- **Veterans Benefits** (VA-approved programs)
- **Registered Apprenticeship** (DOL-funded)

### Support Services Provided

**Career Services:**
- Career coaching and counseling
- Resume and interview preparation
- Job search assistance
- Employer connections and job fairs
- LinkedIn profile optimization
- Professional clothing assistance

**Wraparound Services:**
- Transportation assistance (bus passes, gas cards)
- Childcare support (referrals and subsidies)
- Technology access (laptops, internet)
- Textbooks and materials (included in tuition)
- Uniforms and equipment (provided or stipend)
- Emergency financial assistance

**Student Success Support:**
- Academic tutoring and study groups
- Mentorship programs
- Peer support networks
- Mental health and counseling referrals
- Case management (for high-need students)
- Post-graduation alumni network

---

## Indiana DWD/WorkOne Partnership

### WorkOne Referral Process

1. **Eligibility Determination:** WorkOne counselor assesses WIOA/WRG eligibility
2. **Program Selection:** Student selects ETPL-approved program
3. **Enrollment:** Elevate for Humanity processes enrollment
4. **Training:** Student completes program with support services
5. **Job Placement:** Career services assist with employment
6. **Follow-Up:** 12-month tracking for outcomes reporting

### WorkOne Contact Information

**Indiana DWD:**  
Phone: 1-800-891-6499  
Website: www.in.gov/dwd

**WorkOne Indianapolis:**  
Phone: (317) 684-2400  
Address: [WorkOne Center Address]

---

## Program Outcomes Reporting

Elevate for Humanity commits to:
- Quarterly performance reporting to Indiana DWD
- Annual ETPL recertification and updates
- Student outcome tracking (employment, wages, retention)
- Continuous quality improvement
- Employer satisfaction surveys
- Graduate follow-up at 6 and 12 months

---

## Additional Documentation Available Upon Request

- Detailed curriculum for each program
- Instructor resumes and credentials
- Facility photos and equipment inventory
- Clinical/externship partnership agreements
- Student handbook and policies
- Tuition and fee schedules
- Financial aid information
- Accreditation documentation
- Insurance certificates
- Business licenses and registrations

---

## Certification

I certify that the information contained in this ETPL submission package is true and accurate to the best of my knowledge. Elevate for Humanity commits to maintaining program quality, meeting performance standards, and serving WIOA/WRG-eligible students with excellence.

**Signature:** _________________________  
**Name:** [Executive Director Name]  
**Title:** Executive Director  
**Date:** November 2024

---

## Contact for Questions

For questions about this submission or any programs:

**Elevate for Humanity**  
Email: elevate4humanityedu@gmail.com  
Phone: (317) 314-3757  
Website: elevateforhumanity.institute

**ETPL Coordinator:** [Name]  
**Direct Line:** [Phone]  
**Email:** [Email]

---

*This document serves as the master submission package for Indiana ETPL listing. Individual program syllabi are attached as separate documents. All information is current as of November 2024 and subject to updates as programs evolve to meet industry needs.*

**Document Version:** 1.0  
**Last Updated:** November 28, 2024  
**Next Review Date:** November 2025
