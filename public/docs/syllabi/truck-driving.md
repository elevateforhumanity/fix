# CDL / Truck Driving
## Program Syllabus

**Provider:** Elevate for Humanity  
**ETPL Provider ID:** 10000949  
**Program Code:** CDL-TRUCK  
**Duration:** 4-6 Weeks (160 Hours)  
**Delivery Method:** Classroom + Range + Road Training  
**Funding:** Workforce Grants • Employer Sponsors • WIOA  

---

## Program Overview

Get your Commercial Driver's License in just 4-6 weeks! 160-hour program includes classroom instruction, range practice, and real road experience. Learn pre-trip inspections, backing maneuvers, highway driving, and DOT regulations. Graduate with CDL Class A and immediate job placement assistance with trucking companies offering $50K+ starting salaries.

---

## Learning Objectives

1. Operate Class A commercial motor vehicles safely
2. Perform pre-trip and post-trip inspections
3. Execute backing maneuvers and docking procedures
4. Navigate highways and urban environments
5. Apply DOT regulations and hours of service rules
6. Maintain logs and documentation
7. Handle emergency situations
8. Pass CDL written and skills exams

---

## Course Outline

### Week 1: Classroom Instruction (40 hours)
- Commercial motor vehicle overview
- Federal Motor Carrier Safety Regulations (FMCSR)
- Hours of service and logbook requirements
- Vehicle systems and components
- Air brake systems
- Hazardous materials awareness
- Pre-trip inspection procedures
- Map reading and trip planning

### Week 2: Range Training - Backing & Maneuvers (40 hours)
- Straight line backing
- Offset backing (left and right)
- Alley dock backing
- Parallel parking
- Coupling and uncoupling
- Sliding tandems
- Brake checks and adjustments

### Week 3: Road Training - Basic Skills (40 hours)
- Starting and stopping
- Shifting techniques (manual transmission)
- Turning and lane changes
- Speed management
- Following distance
- Railroad crossings
- Urban driving

### Week 4: Road Training - Advanced Skills (40 hours)
- Highway driving
- Mountain and grade driving
- Adverse weather conditions
- Night driving
- Defensive driving techniques
- Emergency maneuvers
- Fuel management

---

## Assessment Methods

- Written exams (general knowledge, air brakes, combination vehicles)
- Pre-trip inspection demonstration
- Skills test (backing maneuvers)
- Road test evaluation
- Indiana BMV CDL exams

---

## Indiana CDL Requirements

To obtain CDL Class A, students must:
1. Be at least 21 years old (18 for intrastate)
2. Hold valid Indiana driver's license
3. Pass DOT physical examination
4. Pass drug screening
5. Pass written exams (general knowledge, air brakes, combination)
6. Pass pre-trip inspection
7. Pass skills test (backing maneuvers)
8. Pass road test

---

## Required Materials

- DOT physical exam (arranged through program)
- Drug screening (arranged through program)
- CDL permit (obtained during program)
- Study materials (provided)
- Safety equipment (provided)

---

## Instructor Qualifications

- Valid CDL Class A license
- Minimum 5 years commercial driving experience
- CDL instructor certification
- Clean driving record
- DOT medical certification

---

## Career Outcomes

- Over-the-Road (OTR) Truck Driver
- Regional Truck Driver
- Local Delivery Driver
- Tanker Driver
- Flatbed Driver
- Team Driver

**Average Starting Wage:** $50,000-$65,000/year  
**Experienced Wage:** $65,000-$85,000+/year  
**Job Placement Rate:** 95% within 30 days

---

## Program Completion Requirements

- Complete all 160 hours
- Pass all written exams
- Demonstrate competency in all skills
- Pass pre-trip inspection
- Pass backing maneuvers test
- Pass road test
- Obtain CDL Class A license

---

## Employer Partnerships

Program partners with major carriers:
- Schneider National
- Werner Enterprises
- Swift Transportation
- Prime Inc.
- CR England
- Local and regional carriers

---

## Contact Information

**Elevate for Humanity**  
Website: elevateforhumanity.institute  
Phone: (317) 314-3757

---

**Last Updated:** November 2024  
**ETPL Approved:** Yes
